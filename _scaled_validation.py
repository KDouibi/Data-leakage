...
This script uses the same _validation fonctionalities as  model selection/ from scikit-learn,
...
def scaled_cross_validate(estimator, X, y=None, *, groups=None, scoring=None, cv=None,
    ...
    results = parallel(
        delayed(_fit_and_score_customized)(
...
def scale_data (xtr, xtst):
    ...
    from maa.config.ml_config import STRATEGY_SCALER
    scaler = set_scaler(STRATEGY_SCALER)
    saved_shape = xtr.shape
    savedshape_tst = xtst.shape
    print('previous shape',saved_shape)
    xtr_scaled_ = scaler.fit_transform(xtr.reshape(-1, 1))
    xtst_scaled_ = scaler.fit_transform(xtst.reshape(-1, 1))
    xtr_scaled = xtr_scaled_.reshape(saved_shape)
    xtst_scaled = xtst_scaled_.reshape(savedshape_tst)
    print('new reshape', xtr_scaled.reshape(saved_shape))
    #print(xtr)
    #print(xtr_scaled)
    return xtr_scaled, xtst_scaled

def _fit_and_score_customized(estimator, X, y, scorer, train, test, verbose,
    ...
    ###########################################################
    #scale the train and test data separately withis each Fold for CV
    #from maa.analysis.ml import scaled_stratifiedkfold
    #sc1 = ScaledStratifiedKFold(3, n_splits, strategy_scaler, acquisition_version)
    X_train_, y_train = _safe_split(estimator, X, y, train)
    X_test_, y_test = _safe_split(estimator, X, y, test, train)
    print('I am scaling here inside fit and score customized!')
    #print('Train before scale', X_train_)
    X_train, X_test = scale_data(X_train_, X_test_) #TODO il faut que la sortie Sparse matrix or List
    #print('Train after scale', X_train)
    ###########################################################
